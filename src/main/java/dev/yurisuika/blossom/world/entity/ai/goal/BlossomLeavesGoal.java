...
public class BlossomLeavesGoal extends Goal {
    ...
    public boolean checkFilters() {
        ...
        if (Option.getWhitelist()) {
            Arrays.stream(Option.getDimensionWhitelist()).filter(entry -> Objects.equals(entry, entity.getCommandSenderWorld().registryAccess().registryOrThrow(Registry.DIMENSION_TYPE_REGISTRY).getKey(dimension).toString())).map(entry -> true).forEach(whitelist::set);
            Arrays.stream(Option.getBiomeWhitelist()).filter(entry -> Objects.equals(entry, entity.getCommandSenderWorld().registryAccess().registryOrThrow(Registry.BIOME_REGISTRY).getKey(biome).toString())).map(entry -> true).forEach(whitelist::set);
        }

        AtomicBoolean blacklist = new AtomicBoolean(true);
        if (Option.getBlacklist()) {
            Arrays.stream(Option.getDimensionBlacklist()).filter(entry -> Objects.equals(entry, entity.getCommandSenderWorld().registryAccess().registryOrThrow(Registry.DIMENSION_TYPE_REGISTRY).getKey(dimension).toString())).map(entry -> false).forEach(blacklist::set);
            Arrays.stream(Option.getBiomeBlacklist()).filter(entry -> Objects.equals(entry, entity.getCommandSenderWorld().registryAccess().registryOrThrow(Registry.BIOME_REGISTRY).getKey(biome).toString())).map(entry -> false).forEach(blacklist::set);
        }
        ...
        if (hasBlossomedLongEnough()) {
            ...
            if (blockPos != null) {
                ...
                if (blockState.getBlock() == Blocks.OAK_LEAVES) {
                    ...
                    entity.getCommandSenderWorld().setBlockAndUpdate(blockPos, BlossomBlocks.FLOWERING_OAK_LEAVES.get().defaultBlockState()
            ...
            for (int j = 0; j < distance; ++j) {
                for (int k = 0; k <= j; k = k > 0 ? -k : 1 - k) {
                    for (int l = k < j && k > -j ? j : 0; l <= j; l = l > 0 ? -l : 1 - l) {
                        ...
                        if (blockPos.closerThan(mutableBlockPos, distance) && predicate.test(entity.getCommandSenderWorld().getBlockState(mutableBlockPos))) {
