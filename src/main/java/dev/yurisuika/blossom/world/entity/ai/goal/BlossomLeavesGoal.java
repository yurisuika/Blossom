...
public class BlossomLeavesGoal extends Goal {
    ...
    public BlossomLeavesGoal(Bee bee) {
        ...
        this.validBlocks = state -> state.is(Blocks.OAK_LEAVES);
        this.goalChance = Option.getBlossomingChance();
    }

    public boolean checkFilters() {
        Holder<DimensionType> dimension = entity.getLevel().dimensionTypeRegistration();
        ...
        if (Option.getWhitelist()) {
            Arrays.stream(Option.getDimensionWhitelist()).filter(entry -> entry.startsWith("#") && dimension.is(TagKey.create(Registry.DIMENSION_TYPE_REGISTRY, new ResourceLocation(entry.substring(1)))) || Objects.equals(entry, dimension.unwrapKey().get().location().toString())).map(entry -> true).forEach(whitelist::set);
            Arrays.stream(Option.getBiomeWhitelist()).filter(entry -> entry.startsWith("#") && biome.is(TagKey.create(Registry.BIOME_REGISTRY, new ResourceLocation(entry.substring(1)))) || Objects.equals(entry, biome.unwrapKey().get().location().toString())).map(entry -> true).forEach(whitelist::set);
        }

        AtomicBoolean blacklist = new AtomicBoolean(true);
        if (Option.getBlacklist()) {
            Arrays.stream(Option.getDimensionBlacklist()).filter(entry -> entry.startsWith("#") && dimension.is(TagKey.create(Registry.DIMENSION_TYPE_REGISTRY, new ResourceLocation(entry.substring(1)))) || Objects.equals(entry, dimension.unwrapKey().get().location().toString())).map(entry -> false).forEach(blacklist::set);
            Arrays.stream(Option.getBiomeBlacklist()).filter(entry -> entry.startsWith("#") && biome.is(TagKey.create(Registry.BIOME_REGISTRY, new ResourceLocation(entry.substring(1)))) || Objects.equals(entry, biome.unwrapKey().get().location().toString())).map(entry -> false).forEach(blacklist::set);
        }
        ...
        if (hasBlossomedLongEnough()) {
            ...
            if (blockPos != null) {
                ...
                if (blockState.getBlock() == Blocks.OAK_LEAVES) {
                    ...
                    entity.getLevel().setBlockAndUpdate(blockPos, BlossomBlocks.FLOWERING_OAK_LEAVES.get().defaultBlockState()
            ...
            for (int j = 0; j < distance; ++j) {
                for (int k = 0; k <= j; k = k > 0 ? -k : 1 - k) {
                    for (int l = k < j && k > -j ? j : 0; l <= j; l = l > 0 ? -l : 1 - l) {
                        ...
                        if (blockPos.closerThan(mutableBlockPos, distance) && predicate.test(entity.getLevel().getBlockState(mutableBlockPos))) {
