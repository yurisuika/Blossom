        ...
        dispatcher.register(literal("blossom")
                .then(literal("config")
                        ...
                        .then(literal("reload")
                                .executes(context -> {
                                    loadConfig();
                                    context.getSource().sendFeedback(new TranslatableText("commands.config.reload"), true);
                                    return 1;
                                })
                        )
                        .then(literal("reset")
                                .executes(context -> {
                                    ...
                                    saveConfig();
                                    context.getSource().sendFeedback(new TranslatableText("commands.config.reset"), true);
                                    return 1;
                                })
                        )
                )
                .then(literal("query")
                        .then(literal("exposed")
                                .executes(context -> {
                                    context.getSource().sendFeedback(new TranslatableText("commands.query.exposed", config.exposed), false);
                                    return 1;
                                })
                        )
                        .then(literal("rate")
                                .executes(context -> {
                                    context.getSource().sendFeedback(new TranslatableText("commands.query.rate", config.rate), false);
                                    return 1;
                                })
                        )
                        .then(literal("count")
                                .executes(context -> {
                                    context.getSource().sendFeedback(new TranslatableText("commands.query.count", config.count.min, config.count.max), false);
                                    return 1;
                                })
                        )
                )
                        ...
                        .then(literal("exposed")
                                .then(argument("value", BoolArgumentType.bool())
                                        .executes(context -> {
                                            ...
                                            saveConfig();
                                            context.getSource().sendFeedback(new TranslatableText("commands.set.exposed", config.exposed), true);
                                            return 1;
                                        })
                                )
                        )
                        .then(literal("rate")
                                .then(CommandManager.argument("value", IntegerArgumentType.integer(1))
                                        .executes(context -> {
                                            ...
                                            saveConfig();
                                            context.getSource().sendFeedback(new TranslatableText("commands.set.rate", config.rate), true);
                                            return 1;
                                        })
                                )
                        )
                                ...
                                .then(CommandManager.argument("min", IntegerArgumentType.integer(1, 64))
                                        .then(CommandManager.argument("max", IntegerArgumentType.integer(1, 64))
                                                .executes(context -> {
                                                    ...
                                                    saveConfig();
                                                    context.getSource().sendFeedback(new TranslatableText("commands.set.count", config.count.min, config.count.max), true);
                                                    return 1;
                                                })
                                        )
                                )
