        ...
        dispatcher.register(literal("blossom")
                ...
                .then(literal("value")
                        ...
                        .then(literal("propagation")
                                .executes(context -> {
                                    context.getSource().sendFeedback(new TranslatableText("commands.blossom.value.propagation.query", config.value.propagation.chance), false);
                                    return 1;
                                })
                                .then(CommandManager.argument("chance", FloatArgumentType.floatArg(0.0F, 1.0F))
                                        .executes(context -> {
                                            ...
                                            saveConfig();
                                            context.getSource().sendFeedback(new TranslatableText("commands.blossom.value.propagation.set", config.value.propagation.chance), true);
                                            return 1;
                                        })
                                )
                        )
                        .then(literal("fertilization")
                                .executes(context -> {
                                    context.getSource().sendFeedback(new TranslatableText("commands.blossom.value.fertilization.query", config.value.fertilization.chance), false);
                                    return 1;
                                })
                                .then(CommandManager.argument("chance", FloatArgumentType.floatArg(0.0F, 1.0F))
                                        .executes(context -> {
                                            ...
                                            saveConfig();
                                            context.getSource().sendFeedback(new TranslatableText("commands.blossom.value.fertilization.set", config.value.fertilization.chance), true);
                                            return 1;
                                        })
                                )
                        )
                        .then(literal("pollination")
                                .executes(context -> {
                                    context.getSource().sendFeedback(new TranslatableText("commands.blossom.value.pollination.query", config.value.pollination.age), false);
                                    return 1;
                                })
                                .then(CommandManager.argument("age", IntegerArgumentType.integer(0, 7))
                                        .executes(context -> {
                                            ...
                                            saveConfig();
                                            context.getSource().sendFeedback(new TranslatableText("commands.blossom.value.pollination.set", config.value.pollination.age), true);
                                            return 1;
                                        })
                                )
                        )
                        .then(literal("fruit")
                                .executes(context -> {
                                    context.getSource().sendFeedback(new TranslatableText("commands.blossom.value.fruit.query", config.value.fruit.bonus, config.value.fruit.chance), false);
                                    return 1;
                                })
                                .then(CommandManager.argument("bonus", IntegerArgumentType.integer(0))
                                        .then(CommandManager.argument("chance", FloatArgumentType.floatArg(0.0F, 1.0F))
                                                .executes(context -> {
                                                    ...
                                                    saveConfig();
                                                    context.getSource().sendFeedback(new TranslatableText("commands.blossom.value.fruit.set", bonus, chance), true);
                                                    return 1;
                                                })
                                        )
                                )
                ...
                .then(literal("filter")
                        ...
                        .then(literal("temperature")
                                .executes(context -> {
                                    context.getSource().sendFeedback(new TranslatableText("commands.blossom.filter.temperature.query", config.filter.temperature.min, config.filter.temperature.max), false);
                                    return 1;
                                })
                        ...
                        .then(literal("downfall")
                                .executes(context -> {
                                    context.getSource().sendFeedback(new TranslatableText("commands.blossom.filter.downfall.query", config.filter.downfall.min, config.filter.downfall.max), false);
                                    return 1;
                                })
                                .then(argument("min", FloatArgumentType.floatArg(0.0F, 1.0F))
                                        .then(argument("max", FloatArgumentType.floatArg(0.0F, 1.0F))
                                                .executes(context -> {
                                                    ...
                                                    saveConfig();
                                                    context.getSource().sendFeedback(new TranslatableText("commands.blossom.filter.downfall.set", min, max), false);
                                                    return 1;
                                                })
                                        )
                                )
                        ...
                        .then(literal("dimension")
                                .then(literal("whitelist")
                                        .executes(context -> {
                                            context.getSource().sendFeedback(new TranslatableText("commands.blossom.filter.dimension.whitelist.query", String.join(", ", config.filter.dimension.whitelist)), false);
                                            return 1;
                                        })
                                        .then(literal("add")
                                ...
                                .then(literal("blacklist")
                                        .executes(context -> {
                                            context.getSource().sendFeedback(new TranslatableText("commands.blossom.filter.dimension.blacklist.query", String.join(", ", config.filter.dimension.blacklist)), false);
                                            return 1;
                                        })
                                        .then(literal("add")
                                                .then(argument("dimension", DimensionArgumentType.dimension())
                        ...
                        .then(literal("biome")
                                .then(literal("whitelist")
                                        .executes(context -> {
                                            context.getSource().sendFeedback(new TranslatableText("commands.blossom.filter.biome.whitelist.query", String.join(", ", config.filter.biome.whitelist)), false);
                                            return 1;
                                        })
                                        .then(literal("add")
                                ...
                                .then(literal("blacklist")
                                        .executes(context -> {
                                            context.getSource().sendFeedback(new TranslatableText("commands.blossom.filter.biome.blacklist.query", String.join(", ", config.filter.biome.blacklist)), false);
                                            return 1;
                                        })
                                        .then(literal("add")
                                                .then(argument("biome", RegistryPredicateArgumentType.registryPredicate(Registry.BIOME_KEY))
                ...
                .then(literal("toggle")
                        ...
                        .then(literal("whitelist")
                                .executes(context -> {
                                    context.getSource().sendFeedback(new TranslatableText("commands.blossom.toggle.whitelist.query", config.toggle.whitelist), false);
                                    return 1;
                                })
                                .then(argument("value", BoolArgumentType.bool())
                                        .executes(context -> {
                                            ...
                                            saveConfig();
                                            context.getSource().sendFeedback(new TranslatableText("commands.blossom.toggle.whitelist.set", config.toggle.whitelist), false);
                                            return 1;
                                        })
                                )
                        )
                        .then(literal("blacklist")
                                .executes(context -> {
                                    context.getSource().sendFeedback(new TranslatableText("commands.blossom.toggle.blacklist.query", config.toggle.blacklist), false);
                                    return 1;
                                })
                                .then(argument("value", BoolArgumentType.bool())
                                        .executes(context -> {
                                            ...
                                            saveConfig();
                                            context.getSource().sendFeedback(new TranslatableText("commands.blossom.toggle.blacklist.set", config.toggle.blacklist), false);
                                            return 1;
                                        })
                                )
                        )
